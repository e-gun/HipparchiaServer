...
def simplesearchworkwithexclusion(seeking, workdbname, searchobject, cursor, templimit=None):
	...
	for p in so.psgexclusions:
		if workdbname in p:
			restrictions.append(atsignwhereclauses(p, '<>', so.authorswhere))
...
def oldsubstringsearch(seeking, workdbname, searchobject, cursor, templimit=None):
	"""
	...
	if len(workdbname) == 10:
		...
		wid = workdbname[0:10]
		d = [wid, seeking]
		w = atsignwhereclauses(workdbname, '=', so.authorswhere)
