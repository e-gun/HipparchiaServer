...
def wkgconfigurewhereclausedata(authorandworklist, workdict, searchobject):
	"""
	...
	# elapsed 1.1920928955078125e-05
	...
	for n in nonliteraryworks:
		try:
			nonliteryauthors[n[0:6]] = nonliteryauthors[n[0:6]].union(workdict[n].lines())
		except KeyError:
			nonliteryauthors[n[0:6]] = workdict[n].lines()
	...
	for i in ixlist:
		if i[1]['listofboundaries']:
			try:
				partialworks[i[0][0:6]]['listofboundaries'] = list(
					set(partialworks[i[0][0:6]]['listofboundaries'] + i[1]['listofboundaries']))
				partialworks[i[0][0:6]]['listofomissions'] = list(
					set(partialworks[i[0][0:6]]['listofomissions'] + i[1]['listofomissions']))
			except KeyError:
				partialworks[i[0][0:6]] = i[1]
		if i[1]['listofomissions']:
			try:
	...
	for i in partialworks:
		...
		if not supplement:
			...
			indexedauthorlist[i]['where']['listofboundaries'] = list(
				set(indexedauthorlist[i]['where']['listofboundaries'] + partialworks[i]['listofboundaries']))
			indexedauthorlist[i]['where']['listofomissions'] = list(
				set(indexedauthorlist[i]['where']['listofomissions'] + partialworks[i]['listofomissions']))
